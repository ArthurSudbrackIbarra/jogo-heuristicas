...
\begin{enumerate}
  ...
  \item The narrator announces the mission (the task to be completed).
...
\caption{Paired scenarios for heuristic 8 (Aesthetic and Minimalist Design): (a) bad scenario --- the player's mission and the cluttered interface they meet (left), and the narrator's commentary after the task is completed (right); (b) good scenario --- the same mission in a minimal interface (left), and the narrator's commentary after the task is completed (right).}
...
\begin{longtable}[c]{c|p{2.3cm}|p{2.5cm}|p{4.0cm}|p{4.0cm}}
  ...
  \hline
  5 & Error Prevention & Delete the account from settings. & A single click permanently deletes the account, with no warning or confirmation. & Several protective layers --- a de-emphasized button, a warning dialog, a type-to-confirm field, and an acknowledgement checkbox --- prevent accidental deletion. \\
  \hline
  ...
  \hline
  9 & Help Users Recognize, Diagnose, and Recover from Errors & Fix the form error and complete registration. & A generic message ``Error 422: Unprocessable Entity'' tells the user nothing about which field is wrong or how to fix it. & Inline error messages highlight the exact field and explain the specific problem (for example, ``Password must be at least 8 characters''). \\
  \hline
