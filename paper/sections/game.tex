...
The ten heuristics covered by the game follow the consolidated formulation by Nielsen~\cite{nielsen1994heuristic,nielsen2024tenheuristics}. Each heuristic is paired with a concrete, self-contained task that can be carried out by the player in a few clicks within a single screen. Table~\ref{tab:heuristics} lists the heuristics in the order in which they appear in the game and the task used in each pair of scenarios.

\begin{table}[ht]
  \caption{The ten Nielsen heuristics and the concrete task used in each pair of scenarios.}
  \label{tab:heuristics}
  \centering
  \begin{tabular}{c|p{5cm}|p{8cm}}
    \hline
    \textbf{\#} & \textbf{Heuristic} & \textbf{Task} \\
    \hline
    1 & Visibility of System Status & Send a message to your friend Ana about dinner tonight. \\
    \hline
    2 & Match Between System and the Real World & Save your document before closing. \\
    \hline
    3 & User Control and Freedom & You accidentally opened a plan-upgrade wizard --- exit without completing it. \\
    \hline
    4 & Consistency and Standards & Navigate through all four checkout form steps. \\
    \hline
    5 & Error Prevention & Delete your account from settings. \\
    \hline
    6 & Recognition Rather Than Recall & Bold the word ``important'' in the document. \\
    \hline
    7 & Flexibility and Efficiency of Use & Mark all three unread notifications as read. \\
    \hline
    8 & Aesthetic and Minimalist Design & Find and click the ``Buy Now'' button for the Laptop XZ Pro. \\
    \hline
    9 & Help Users Recognise, Diagnose, and Recover from Errors & Fix the form error and complete registration. \\
    \hline
    10 & Help and Documentation & Enable Two-Factor Authentication (2FA) in Security Settings. \\
    \hline
  \end{tabular}
\end{table}
...
A few features were deliberately left out of the artefact, and it is worth naming them explicitly. The game does not adapt to the player: all participants see the same ten heuristics in the same order, and there is no adaptive difficulty. The game also does not assess learning: the player cannot ``get a heuristic wrong'' --- they complete the task and the narrator explains what just happened, and there is no quiz or scoring of decisions. There is no collaborative or social mode; the experience is single-player and single-session. Replaying the game shows the same content. The artefact is intentionally a content-delivery and reflection-scaffolding device that complements an instructor, rather than a stand-alone teaching system or an assessment instrument.
